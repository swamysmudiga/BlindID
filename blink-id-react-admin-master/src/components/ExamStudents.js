...
import StudentService from '../services/exam_students.service'; 
...
const handleSearchChange = async e => {
  ...
  const handleAddUser = async user => {
    try {
      ...
      await StudentService.create({ groupid: id, user_id:user.id }, token); // Pass the user object
  ...
  return (
    <div className="container">
      <h2>Exam - {examDetails && examDetails.name} - Students</h2>
      ...
      <table className='table table-striped'>
        ...
        <tbody>
          {examDetails && examDetails.users.map((user, index) => (
            <tr key={user.id}>
              <td>{index + 1}</td>
              <td>   <img
                          src={
